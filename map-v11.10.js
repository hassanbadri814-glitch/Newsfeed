...
(function(){
  ...
  function appendItemsChunked(items){
    ...
    function renderChunk(){
      ...
      if(index < items.length){
        VOD._renderChunkTimer = requestAnimationFrame(renderChunk);
      } else {
        VOD._renderChunkTimer = null;
        bindGridEvents();
      }
  ...
  async function loadCatalog(loadMore){
    ...
    if(cached){
      ...
      appendItemsChunked(cached);
      VOD.currentItems = VOD.currentItems.concat(cached);
      updateHeaderCount();
